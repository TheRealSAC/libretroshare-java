...
bool p3ChatService::checkForMessageSecurity(RsChatMsgItem *ci)
{
	...
	if (ci->message.length() > 60000 && (ci->chatFlags & RS_CHAT_FLAG_LOBBY))
	{
		wchar_t tmp[300];
		mbstowcs(tmp, rsPeers->getPeerName(ci->PeerId()).c_str(), 299);

		ci->message = std::wstring(L"**** Security warning: Message bigger than 60000 characters, forwarded to you by ") + tmp + L", dropped. ****";
		return false;
	}
